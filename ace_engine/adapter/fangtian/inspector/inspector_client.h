...
using FastLinuxErrorCallback = std::function<void(const std::string &jsonStr)>;
...
public:
    ...
    void RegisterFastLinuxErrorCallback(FastLinuxErrorCallback&& callback);
    ...
    void CallFastLinuxErrorCallback(const std::string& jsonStr)
    {
        if (onError_) {
            onError_(jsonStr);
        }
...
private:
    ...
    FastLinuxErrorCallback onError_;
};
