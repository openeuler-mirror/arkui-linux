...
const array1ObsObjPropsPU = tsuite("Partial update: Array of Observed Object and Properties", () => {
  ...
  tcase("Array of ClassA", () => {
    ...
    let parentViewProp: ObservedPropertyObjectAbstractPU<ClassA[]>
      = new ObservedPropertyObjectPU<ClassA[]>(arr, parentView, "parentViewProp");
    ...
    let childViewProps: ObservedPropertyObjectAbstractPU<ClassA>[] = [];
    [0, 1, 2].forEach((i) =>
      childViewProps.push(
        new SynchedPropertyNesedObjectPU<ClassA>(arr[i], childViews[i], `${i}`))
    );
    ...
    childViewProps.push(
      new SynchedPropertyNesedObjectPU<ClassA>(arr[3], childViews[3], `${3}`)
    );
