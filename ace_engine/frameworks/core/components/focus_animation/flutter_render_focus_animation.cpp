...
#include "core/components/focus_animation/flutter_render_focus_animation.h"
...
void FlutterRenderFocusAnimation::Paint(RenderContext& context, const Offset& offset)
{
    ...
    if (SystemProperties::GetDeviceType() == DeviceType::TV) {
        PaintTVFocus(skCanvas);
    } else if (SystemProperties::GetDeviceType() == DeviceType::PHONE) {
        PaintPhoneFocus(skCanvas);
    }
}

void FlutterRenderFocusAnimation::PaintGlow(SkCanvas* skCanvas, SkPaint& paint, int32_t padding) const
{
...
RenderLayer FlutterRenderFocusAnimation::GetRenderLayer()
{
    if (!layer_) {
...
void FlutterRenderFocusAnimation::PaintClipRect(SkCanvas* skCanvas, double offset) const
{
...
void FlutterRenderFocusAnimation::PaintTVFocus(SkCanvas* skCanvas)
{
...
void FlutterRenderFocusAnimation::PaintPhoneFocus(SkCanvas* skCanvas)
{
    double radiusX = 0.0;
    double radiusY = 0.0;
    if (!isIndented_) {
