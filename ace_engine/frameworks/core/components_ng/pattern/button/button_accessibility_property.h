...
public:
    ...
    ~ButtonAccessibilityProperty() override = default;
    ...
    {
        if (propText_.has_value()) {
            if (NearEqual(propText_.value(), text)) {
                return;
            }
        }
...
private:
    ...
    ACE_DISALLOW_COPY_AND_MOVE(ButtonAccessibilityProperty);
};
} // namespace OHOS::Ace::NG
