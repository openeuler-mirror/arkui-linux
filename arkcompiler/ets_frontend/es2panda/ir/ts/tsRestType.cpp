...
void TSRestType::Iterate(const NodeTraverser &cb) const
{
    cb(type_);
}

void TSRestType::Dump(ir::AstDumper *dumper) const
{
    dumper->Add({{"type", "TSRestType"}, {"typeAnnotation", type_}});
}
