...
bool MutexDoNotCheckOnDeadlock()
{
    return g_deadlockFlag;
}

void MutexIgnoreChecksOnDeadlock()
{
    g_deadlockFlag = true;
}
...
int32_t GetWaiters(struct fmutex *const m)
{
    ...
    return static_cast<uint32_t>(ATOMIC_LOAD(&m->state_and_waiters_, memory_order_relaxed)) >> WAITER_SHIFT;
}
...
void MutexDestroy(struct fmutex *m)
{
    ...
    if (!MutexDoNotCheckOnDeadlock()) {
        ...
        LOG_MESSAGE(WARNING, "Deadlock detected, ignoring Mutex");
    }
#endif  // PANDA_TARGET_MOBILE
}
