...
#include "codedataaccessor_fuzzer.h"
...
namespace OHOS {
void CodeDataAccessorFuzzTest(const uint8_t* data, size_t size)
{
    try {
        ...
        for (const auto &header : panda_file.GetIndexHeaders()) {
            ...
            for (const auto &id : methods) {
                ...
                if (value.has_value()) {
                    panda::panda_file::CodeDataAccessor coda(panda_file, value.value());
                }
            }
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::CodeDataAccessorFuzzTest(data, size);
    return 0;
}
