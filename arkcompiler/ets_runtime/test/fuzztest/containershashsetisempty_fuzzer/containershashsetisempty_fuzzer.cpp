...
#include "containershashsetisempty_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersHashSetIsEmptyFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        EcmaRuntimeCallInfo *callInfoIsEmpty = CreateEcmaRuntimeCallInfo(thread, 6);
        callInfoIsEmpty->SetFunction(JSTaggedValue::Undefined());
        callInfoIsEmpty->SetThis(hashSet.GetTaggedValue());
        [[maybe_unused]] JSTaggedValue resultIsEmpty = ContainersHashSet::IsEmpty(callInfoIsEmpty);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersHashSetIsEmptyFuzzTest(data, size);
    return 0;
}