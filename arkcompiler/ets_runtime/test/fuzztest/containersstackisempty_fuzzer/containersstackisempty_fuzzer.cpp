...
#include "containersstackisempty_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersStackFuzzIsEmptyTest(const uint8_t* data, size_t size)
    {
...

        {
            ...
            callInfo->SetFunction(JSTaggedValue::Undefined());
            callInfo->SetThis(stack.GetTaggedValue());
            callInfo->SetCallArg(0, JSTaggedValue());
            ContainersStack::Pop(callInfo);
            ContainersStack::IsEmpty(callInfo);
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersStackFuzzIsEmptyTest(data, size);
    return 0;
}