...
#include "containersqueueforeach_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersQueueForEachFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        JSHandle<JSAPIQueue> dlist = CreateJSAPIQueue(thread);
        {
            JSHandle<GlobalEnv> env = thread->GetEcmaVM()->GetGlobalEnv();
            ...
            ContainersQueue::ForEach(callInfo);
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersQueueForEachFuzzTest(data, size);
    return 0;
}