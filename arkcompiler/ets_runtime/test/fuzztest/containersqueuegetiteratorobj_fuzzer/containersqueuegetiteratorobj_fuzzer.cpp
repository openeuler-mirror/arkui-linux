...
#include "containersqueuegetiteratorobj_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersQueueGetIteratorObjFuzzTest(const uint8_t* data, size_t size)
    {
...

        {
            ...
            callInfo->SetFunction(JSTaggedValue::Undefined());
            callInfo->SetThis(queue.GetTaggedValue());
            ContainersQueue::GetIteratorObj(callInfo);
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersQueueGetIteratorObjFuzzTest(data, size);
    return 0;
}