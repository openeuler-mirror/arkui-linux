...
#include "containersstackforeach_fuzzer.h"
...
namespace OHOS {
    ...
    JSTaggedValue TestForEachFunc([[maybe_unused]] EcmaRuntimeCallInfo *argv)
    {
    ...
    void ContainersStackForEachFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        JSHandle<JSAPIStack> dlist = CreateJSAPIStack(thread);
        {
            JSHandle<GlobalEnv> env = thread->GetEcmaVM()->GetGlobalEnv();
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersStackForEachFuzzTest(data, size);
    return 0;
}