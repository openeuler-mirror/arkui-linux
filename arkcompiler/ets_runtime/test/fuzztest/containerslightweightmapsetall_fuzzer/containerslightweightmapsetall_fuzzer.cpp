...
#include "containerslightweightmapsetall_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersLightWeightMapSetAllFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        JSHandle<JSAPILightWeightMap> oldLightWeightMap = CreateJSAPILightWeightMap(thread);

        EcmaRuntimeCallInfo *callInfo1 = CreateEcmaRuntimeCallInfo(thread, 8);
        callInfo1->SetFunction(JSTaggedValue::Undefined());
        callInfo1->SetThis(oldLightWeightMap.GetTaggedValue());
        ...
        callInfo2->SetCallArg(0, oldLightWeightMap.GetTaggedValue());
        ContainersLightWeightMap::SetAll(callInfo2);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersLightWeightMapSetAllFuzzTest(data, size);
    return 0;
}