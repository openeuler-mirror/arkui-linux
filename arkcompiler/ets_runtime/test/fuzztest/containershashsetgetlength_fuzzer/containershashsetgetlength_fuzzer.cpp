...
#include "containershashsetgetlength_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersHashSetGetLengthFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        EcmaRuntimeCallInfo *callInfoGetLength = CreateEcmaRuntimeCallInfo(thread, 6);
        callInfoGetLength->SetFunction(JSTaggedValue::Undefined());
        callInfoGetLength->SetThis(hashSet.GetTaggedValue());
        [[maybe_unused]] JSTaggedValue resultGetLength = ContainersHashSet::GetLength(callInfoGetLength);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersHashSetGetLengthFuzzTest(data, size);
    return 0;
}