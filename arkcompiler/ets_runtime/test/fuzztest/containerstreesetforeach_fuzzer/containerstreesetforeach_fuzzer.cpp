...
#include "containerstreesetforeach_fuzzer.h"
...
namespace OHOS {
    ...
    void ContainersTreeSetForEachFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        ObjectFactory *factory = thread->GetEcmaVM()->GetFactory();
        JSHandle<JSAPITreeSet> dset = CreateJSAPITreeSet(thread);
        JSHandle<GlobalEnv> env = thread->GetEcmaVM()->GetGlobalEnv();
        ...
        objCallInfo->SetCallArg(1, dset.GetTaggedValue());
        ContainersTreeSet::ForEach(objCallInfo);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::ContainersTreeSetForEachFuzzTest(data, size);
    return 0;
}